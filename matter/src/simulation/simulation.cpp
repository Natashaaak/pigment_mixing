...
void Simulation::createDirectory(){
    ...
    if (!in.is_open()) {
        std::cerr << "ERROR: File mpm.cpp was not found at: " << file_in << std::endl;
        // Try spare path, if SRC_DIR failed
        file_in = "./mpm.cpp"; 
    ...
    if (in && out) {
        out << in.rdbuf();
        std::cout << "Initial setup " << file_in << " was successfully copied." << std::endl;
    }
    else {
